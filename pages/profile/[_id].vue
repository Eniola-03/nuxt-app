    ...
    <div class="container m-auto py-10 px-6">
      <div class="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
        <div>
          ...
          <span class="text-my-blue font-regular pb-6 md:pb-63">{{ moment.createdAt }}</span>
...
const momentId = ref(route.params._id);
const moment = ref({});
...
onMounted(async () => {
  try {
    const response = await fetch("https://eventful-moments-api.onrender.com/api/v1/moment/614556facd07331621f6bb0a", {
      method: 'GET',
      headers: {
    ...
    moment.value = response.data; // Adjusted to match the provided API structure
  } catch (err) {
    console.error(err);
  }
});
